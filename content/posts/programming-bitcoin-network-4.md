...
We've also set up a goal of building a mempool monitor.

In this part we'll face some difficulties that'll make us to change the goal.
...
But don't get disappointed! We'll set a new goal! And let's first try to build a mempool monitor to see why it's too complex.
...
Are there content rules that enforce the content of mempool? **No**. Every has its own
...
your transactions disappears. If this node isn't mining a new block that contains your
transaction, then you transaction is gone forever.
...
That's actually it for the mempool. But how do we see mempool when our node is running?
...
transactions in the mempool. In the end, this is what mempool mintor is for!

### JSON-RPC interface
We're going to add an RPC interface to our node to allow seeing current mempool state,
...
request handlers (in web-frameworks, called controllers) that handle incoming requests.
...
Thus, we need to define a structure that we'll outline the interface:
```go
// rpc/rpc.go
type Node interface {
...
```
We called the structure simply `RPC`. It needs to communicate with a node to get data
from it. Following best Golang practices we're defining `Node` interface that connects
`RPC` and actual `Node`, and defines what methods are available to `RPC`. As I said
above, we need a way to read the mempool from outside the node, this is what `Mempool`
method is for. Let's add it now:
...
being added or removed from the mempool at the moment; we're just making a copy of
...
define types of arguments externally.
...
but you're free to add something else here.
...
### JSON-RPC server
An RPC server is responsible for reading data from a connection, parsing it, handling, 
and sending a reply. Pretty similar to an HTTP server, but we don't need to define
paths.
...
additionally hold a port number so we could run the server on the specified port.
...
transport layer: we can use TCP or HTTP, and it supports any of them.
...
For simplicity, we'll keep this things together and define a new CLI command, `showmempool`:
...
```
Our `rpc.Client` is a thin wrapper around a TCP connection and a JSON-RPC client.
We're saving a TCP connection because we want to close it later, even though
`jsonrpc.NewClient(conn)` also wraps it.

To call RPC methods we're just calling the underlying `jsonrpc.Call` method passing all
the arguments as is:
```go
...
need to know what transaction were mined. And to know this, we need to learn to receive
...
6. `Nonce` is a random number that was used to get the hash of the black that satisfies the difficulty target stored in `Bits`.
...
they have to pay. And it's transaction fees that prevent the network from spamming.
...
**SPV-node**. SPV stands for Simple Payment Verification, which is a method for
...
that builds full chain without downloading full blocks.**

But since we've started the mempool monitor, let's add some simple verification rules
to demonstrate what they should look like. And, of course, feel free to add more rules!
...
```
Now, we can verify transaction before adding them to the mempool:
```go
// node/cmd_tx.go
func (no Node) handleTx(header *protocol.MessageHeader, conn io.ReadWriter) error {
	...
...
becomes and the more difficult it becomes to find new blocks. The `Bits` field stores
a difficulty target, and we need to unpack it.
...
```
First three lines allow us to reverse the content of `Bits` field. Again, we need this
because Bitcoin uses little-endian order for storage, and we want big-endian order here.
...
smaller the exponent, thus there's a gap. The rest three bytes of `Bits` are the first three bytes of the threshold number. The
rest byte are zeros.
...
that we need to hash to get block hash. This is what `MarshalHeader` does (I omitted
...
```
It basically does what discussed above: it gets difficulty target from `Bits` field,
calculates block hash, and checks if the hash is less or equals to the target.
...
1. ['block' verification rules](https://en.bitcoin.it/wiki/Protocol_rules#.22tx.22_messages)
